...
import { ApiError, login, me, signup, type AuthUser } from "@/lib/api";
import { GateLoading } from "@/components/load-state";

const title = "TalentBro | Login or Create Account";
const description =
  "Sign in or create your TalentBro account to measure and improve placement readiness across your institution.";

type AuthMode = "login" | "signup";

export const Route = createFileRoute("/institution-auth")({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (mode === "signup") {
        ...
        await login({ email, password, userType: "institution_staff" });
      }
        ...
        <div>
          <Reveal delay={120}>
            ...
            <p className="mt-5 max-w-sm text-sm leading-relaxed text-muted-foreground">
              Albert, Peter, Daniel, Maya, Ada and Carl interview every candidate for you — you only
              step in for the final decision.
            </p>
          </Reveal>
          ...
          <div key={mode} className="tg-reveal mt-10">
            <h1 className="font-[family-name:var(--font-display)] text-4xl tracking-tight sm:text-5xl">
              {mode === "login" ? (
                ...
                <>
                  Create your account<span className="italic text-muted-foreground">.</span>
                </>
              )}
            </h1>
            <p className="mt-3 text-sm text-muted-foreground">
              {mode === "login"
                ? "Sign in to access your interview panel and candidate dashboard."
