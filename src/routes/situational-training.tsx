...
import {
  ...
  situationalList,
  situationalSkip,
  situationalSkipKeepalive,
  situationalStart,
  type SituationalCategory,
  type SituationalTrainingSession,
} from "@/lib/api";

const title = "TalentBro | Situational Problem Solving Skills";
const description =
  "Build management judgement with real workplace scenarios — conflict, leadership, ethics, decisions and crises. Your guide is Peter — reply with your decision and reasoning, and earn XP.";

export const Route = createFileRoute("/situational-training")({
  head: () => ({
    meta: [
      { title },
  ...
  component: SituationalPage,
});

const COACH_NAME = "Peter";

const CATEGORY_META: {
  slug: SituationalCategory | "";
  label: string;
  icon: typeof Compass;
  chip: string;
}[] = [
  { slug: "", label: "Surprise me", icon: Sparkles, chip: "bg-emerald-500/15 text-emerald-500" },
  {
  ...
  {
    slug: "ethical_dilemma",
    label: "Ethical Dilemma",
    icon: HeartHandshake,
    chip: "bg-teal-500/15 text-teal-500",
  },
  {
    slug: "crisis_management",
...
function deriveStats(sessions: SituationalTrainingSession[]) {
...
function accOf(list: SituationalTrainingSession[]): number {
  if (list.length === 0) return 0;
  return Math.round((list.filter((s) => s.status === "solved").length / list.length) * 100);
}

function deriveCategoryInsights(sessions: SituationalTrainingSession[]): CategoryInsight[] {
...
function SituationalPage() {
  ...
  const [sessions, setSessions] = useState<SituationalTrainingSession[]>([]);
  const [session, setSession] = useState<SituationalTrainingSession | null>(null);
  ...
  const [category, setCategory] = useState<SituationalCategory | "">("");
  const [selectedDetail, setSelectedDetail] = useState<SituationalTrainingSession | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);

  const scrollRef = useRef<HTMLDivElement | null>(null);
  const startingRef = useRef(false);
  const categoryRef = useRef<SituationalCategory | "">("");
  ...
  // unanswered scenario as skipped rather than leaving it in progress.
  useEffect(() => {
    const flush = () => {
      if (activeSessionRef.current) situationalSkipKeepalive(activeSessionRef.current);
    };
    ...
    return () => {
      ...
      if (activeSessionRef.current) {
        void situationalSkip(activeSessionRef.current);
      }
    };
  }, []);
  ...
  async function initialize() {
    let list: SituationalTrainingSession[] = [];
    try {
      list = await situationalList();
    } catch {
      list = [];
    }
    setSessions(list);
    ...
    if (active) {
      // A scenario left unanswered on a previous visit is skipped, never resumed.
      try {
        await situationalSkip(active.id);
      } catch {
        /* best-effort */
      }
      try {
        setSessions(await situationalList());
      } catch {
        /* best-effort refresh */
      }
    }
  ...
  function chooseCategory(slug: SituationalCategory | "") {
  ...
  async function viewSession(id: string) {
    ...
    try {
      const detail = await situationalDetail(id);
  ...
  async function startNew() {
    ...
    setError("");
    try {
      const res = await situationalStart({ category: categoryRef.current });
      setSession(res.session);
      setMessages([{ id: `q-${Date.now()}`, role: "peter", text: res.message }]);
      setInput("");
      setFlow("idle");
      try {
        setSessions(await situationalList());
      ...
      setMessages([
        {
          id: `err-${Date.now()}`,
          role: "peter",
          text: `${COACH_NAME}: I couldn't fetch a new scenario. ${msg} Please try again in a moment.`,
        },
      ]);
    } finally {
  ...
  async function submit(text: string) {
    ...
    setInput("");
    try {
      const res = await situationalChat(session.id, clean);
      const peterMsg: ChatMsg = { id: `a-${Date.now()}`, role: "peter", text: res.reply };
  ...
  // Auto-start the next scenario once one is resolved.
  useEffect(() => {
  ...
  function giveUp() {
    void submit("I give up — please show me how to handle this.");
  }
  ...
  return (
    <div className="relative min-h-svh bg-background text-foreground">
      {/* Deco management environment */}
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[420px] overflow-hidden">
        <div className="absolute -top-24 left-1/2 h-72 w-[42rem] -translate-x-1/2 rounded-full bg-rose-500/15 blur-3xl" />
        <div className="absolute top-16 left-[12%] h-48 w-48 rounded-full bg-violet-500/10 blur-3xl" />
        <div className="absolute top-24 right-[10%] h-48 w-48 rounded-full bg-amber-500/10 blur-3xl" />
        <svg
        ...
        >
          ...
          {Array.from({ length: 10 }).map((_, i) => {
            ...
            const x = 320 + 210 * Math.cos(angle);
            const y = 128 + 210 * Math.sin(angle);
            return (
              <text
                key={i}
                x={x}
                ...
                className="fill-muted-foreground/25 font-mono"
                fontSize="11"
                stroke="none"
              >
                {words[i]}
          ...
          <circle cx="320" cy="128" r="7" className="fill-rose-500/40" stroke="none" />
        </svg>
      </div>
      ...
      <AppNavHeader
        ...
        left={
          <div className="flex items-center gap-2.5">
            ...
            <span>
              <p className="text-sm font-semibold leading-tight">Situational Problem Solving</p>
            </span>
          </div>
        }
      />
        ...
        <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
              Situational Problem Solving · one scenario, one chat
            </p>
            <h1 className="mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">
              Build your management judgement with {COACH_NAME}.
            </h1>
            <p className="mt-1.5 max-w-xl text-[13px] leading-relaxed text-muted-foreground">
              Real workplace scenarios — conflicts, leadership calls, ethical dilemmas, tough
              decisions and crises. Reply with what you would do and why. {COACH_NAME} scores your
              judgement, and a fresh scenario begins automatically.
            </p>
          </div>
        </div>
          ...
          <div className="mb-5 rounded-2xl border border-border bg-card/70 p-4 backdrop-blur">
            ...
            {insights.length === 0 && (
              <p className="mt-3 text-[12px] text-muted-foreground">
                No finished rounds yet — solve or skip a few to see per-skill trends.
              </p>
            )}
          </div>
        )}
              ...
              <div className="flex min-w-0 items-center gap-2.5">
                ...
                <div className="min-w-0">
                  <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
                    {session ? session.category_label : "Scenario"}
                  </p>
                  <h2 className="truncate text-sm font-semibold">
                    {session?.title ?? "Ready when you are — Peter has a scenario for you."}
                  </h2>
                </div>
              </div>
            </div>
                ...
                m.role === "peter" ? (
                  <div key={m.id} className="flex gap-3">
                    ...
                    <div className="max-w-[82%] rounded-2xl rounded-tl-sm border border-border bg-secondary/60 px-4 py-3">
                      <div className="mb-1 font-mono text-[10px] uppercase tracking-widest text-rose-500">
                        {COACH_NAME}
                      </div>
                  ...
                  <div className="rounded-2xl rounded-tl-sm border border-border bg-secondary/60 px-4 py-3">
                    <div className="flex items-center gap-1.5 text-[12px] text-muted-foreground">
                      ...
                      <span className="font-mono text-[10px] uppercase tracking-widest">
                        {COACH_NAME} is evaluating your judgement…
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2 text-sm font-semibold">
                        {result.solved ? "Scenario handled" : "Scenario skipped"}
                        {result.solved && <StarRow count={result.stars} />}
                      </div>
                      <div className="font-mono text-[11px] text-muted-foreground">
                        {result.solved
                          ? `+${result.points} XP ${result.stars >= 4 ? "· sound, reasoned judgement" : "· keep explaining your why"}`
                          : "Approach revealed below"}
                      </div>
                    </div>
                  </div>
            ...
            <div className="border-t border-border bg-background/60 px-4 py-3 backdrop-blur sm:px-5">
              ...
              <div className="flex items-end gap-2">
                <Textarea
                  ...
                  placeholder="Type what you'd do + your reasoning (e.g. I'd split the task, then hear both sides privately)"
                  rows={2}
                ...
                <Button
                  ...
                  disabled={flow !== "idle" || !session}
                  onClick={giveUp}
                  title="Give up and see the recommended approach"
          ...
          <aside className="flex flex-col gap-4">
              ...
              <div className="max-h-[320px] divide-y divide-border overflow-y-auto">
                {sessions.length === 0 && (
                  <p className="px-4 py-6 text-[12px] text-muted-foreground">
                    No rounds yet — your handled and skipped scenarios will show up here.
                  </p>
                )}
                {sessions.map((s) => {
                  return (
            ...
            <div className="rounded-2xl border border-rose-500/30 bg-rose-500/5 p-4">
              ...
              <ol className="mt-3 space-y-2.5 text-[12.5px] text-muted-foreground">
                {[
                  "Peter sets exactly one workplace scenario per chat.",
                  "Reply with your decision — explaining your reasoning shows strong judgement.",
                  "A practical, respectful, ethical call (fast) = XP + stars.",
                  "Handled or gave up? A brand-new scenario starts automatically.",
                ].map((step, i) => (
                  <li key={step} className="flex items-start gap-2">
                    <span className="mt-0.5 grid size-4 shrink-0 place-items-center rounded-full bg-rose-500/20 font-mono text-[9px] text-rose-500">
                      {i + 1}
                    </span>
              ...
              <>
                <DialogHeader>
                  <div className="flex items-center gap-2">
                    ...
                    <DialogTitle className="text-base sm:text-lg">
                      {selectedDetail.title || "Situational Scenario"}
                    </DialogTitle>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="rounded-xl border border-border bg-secondary/40 p-4">
                    <div className="mb-1.5 font-mono text-[10px] uppercase tracking-widest text-rose-500">
                      Scenario
                    </div>
                          ...
                          >
                            <div
                              className={cn(
                                ...
                                t.role === "assistant" ? "text-rose-500" : "opacity-60",
                              )}
                            >
