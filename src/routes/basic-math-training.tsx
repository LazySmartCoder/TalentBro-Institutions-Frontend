...
import {
  ...
  basicMathList,
  basicMathSkip,
  basicMathSkipKeepalive,
  basicMathStart,
  me,
  type AuthUser,
  type BasicMathCategory,
  type BasicMathTrainingSession,
} from "@/lib/api";

const title = "TalentBro | Basic Mathematics";
const description =
  "Build arithmetic speed and accuracy with quick-fire maths drills. Your guide is Albert — answer fast, show your working, and earn XP.";

export const Route = createFileRoute("/basic-math-training")({
  head: () => ({
    meta: [
      { title },
  ...
  component: BasicMathPage,
});

const COACH_NAME = "Albert";

const CATEGORY_META: {
  slug: BasicMathCategory | "";
  label: string;
  icon: typeof Plus;
  chip: string;
}[] = [
  ...
  },
  {
    ...
    label: "Ratio & Average",
    icon: Hash,
    chip: "bg-cyan-500/15 text-cyan-500",
  },
  {
    slug: "mental_math",
    label: "Mental Math",
...
function BasicMathPage() {
  ...
  const [category, setCategory] = useState<BasicMathCategory | "">("");
  const [selectedDetail, setSelectedDetail] = useState<BasicMathTrainingSession | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);

  const scrollRef = useRef<HTMLDivElement | null>(null);
  const startingRef = useRef(false);
  const categoryRef = useRef<BasicMathCategory | "">("");
  ...
  useEffect(() => {
    const flush = () => {
      if (activeSessionRef.current) basicMathSkipKeepalive(activeSessionRef.current);
    };
    ...
    return () => {
      ...
      if (activeSessionRef.current) {
        void basicMathSkip(activeSessionRef.current);
      }
    };
  }, []);
  ...
  function chooseCategory(slug: BasicMathCategory | "") {
  ...
  async function viewSession(id: string) {
    ...
    try {
      const detail = await basicMathDetail(id);
  ...
  async function startNew() {
    ...
    setError("");
    try {
      const res = await basicMathStart({ category: categoryRef.current });
  ...
  async function submit(text: string) {
    ...
    setInput("");
    try {
      const res = await basicMathChat(session.id, clean);
  ...
  return (
    <div className="relative min-h-svh bg-background text-foreground">
      {/* Deco math environment */}
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[420px] overflow-hidden">
        <div className="absolute -top-24 left-1/2 h-72 w-[42rem] -translate-x-1/2 rounded-full bg-emerald-500/15 blur-3xl" />
        <div className="absolute top-16 left-[12%] h-48 w-48 rounded-full bg-teal-500/10 blur-3xl" />
        <div className="absolute top-24 right-[10%] h-48 w-48 rounded-full bg-lime-500/10 blur-3xl" />
        <svg
        ...
        >
          ...
          {Array.from({ length: 10 }).map((_, i) => {
            const digits = [7, 35, 144, 81, 25, 9, 99, 42, 60, 12];
          ...
          <circle cx="320" cy="128" r="7" className="fill-emerald-500/40" stroke="none" />
        </svg>
      </div>
      ...
      <AppNavHeader
        ...
        left={
          <div className="flex items-center gap-2.5">
            ...
            <span>
              <p className="text-sm font-semibold leading-tight">Basic Mathematics</p>
            </span>
          </div>
        }
      />
        ...
        <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
              Basic Maths Training · one question, one chat
            </p>
            <h1 className="mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">
              Build your arithmetic speed with {COACH_NAME}.
            </h1>
            <p className="mt-1.5 max-w-xl text-[13px] leading-relaxed text-muted-foreground">
              Simple, quick-fire maths questions — answer fast, and show your working when you can.{" "}
              {COACH_NAME} rewards both. Solve a question and a fresh one begins automatically.
            </p>
          </div>
        </div>
              ...
              <div className="flex min-w-0 items-center gap-2.5">
                ...
                <div className="min-w-0">
                  ...
                  <h2 className="truncate text-sm font-semibold">
                    {session?.title ?? "Ready when you are — Albert has a question for you."}
                  </h2>
                </div>
              </div>
            </div>
                ...
                m.role === "albert" ? (
                  <div key={m.id} className="flex gap-3">
                    ...
                    <div className="max-w-[82%] rounded-2xl rounded-tl-sm border border-border bg-secondary/60 px-4 py-3">
                      <div className="mb-1 font-mono text-[10px] uppercase tracking-widest text-emerald-500">
                        {COACH_NAME}
                      </div>
                  ...
                  <div className="rounded-2xl rounded-tl-sm border border-border bg-secondary/60 px-4 py-3">
                    <div className="flex items-center gap-1.5 text-[12px] text-muted-foreground">
                      ...
                      <span className="font-mono text-[10px] uppercase tracking-widest">
                        {COACH_NAME} is checking your answer…
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-4 px-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="font-mono text-[11px] text-muted-foreground">
                        {result.solved
                          ? `+${result.points} XP ${result.stars >= 4 ? "· clean, fast working" : "· keep showing your working"}`
            ...
            <div className="border-t border-border bg-background/60 px-4 py-3 backdrop-blur sm:px-5">
              ...
              <div className="flex items-end gap-2">
                <Textarea
                  ...
                  placeholder="Type your answer + working (e.g. 85 - I added 40 + 30, then 7 + 8)"
                  rows={2}
          ...
          <aside className="flex flex-col gap-4">
            ...
            <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-4">
              ...
              <ol className="mt-3 space-y-2.5 text-[12.5px] text-muted-foreground">
                {[
                  "Albert asks exactly one simple maths question per chat.",
                  "Reply with your answer — adding your working shows clean thinking.",
                  "Right answer (fast) = XP + stars. Taking mental shortcuts also pays off.",
                  "Solved or gave up? A brand-new question starts automatically.",
                ].map((step, i) => (
                  <li key={step} className="flex items-start gap-2">
                    <span className="mt-0.5 grid size-4 shrink-0 place-items-center rounded-full bg-emerald-500/20 font-mono text-[9px] text-emerald-500">
                      {i + 1}
                    </span>
                ...
                <DialogHeader>
                  <div className="flex items-center gap-2">
                    ...
                    <DialogTitle className="text-base sm:text-lg">
                      {selectedDetail.title || "Basic Maths Question"}
                    </DialogTitle>
                  </div>
                          ...
                          >
                            <div
                              className={cn(
                                ...
                                t.role === "assistant" ? "text-emerald-500" : "opacity-60",
                              )}
                            >
